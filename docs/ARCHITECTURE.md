...
源组件库可用独立 inline-editing.json 提供公开属性到 Renderer 根内锚点的映射；插件只读取位置，在编辑器浮层挂载真实 C-21。草稿不写 DOM，不改变源样式；单次 updateProps 使用打开时 revision。右侧属性去重且未匹配锚点回到属性栏。协议与生命周期见 [画布内容编辑](INLINE_EDITING.md)。
