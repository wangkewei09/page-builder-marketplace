...
`contextNodeId/contextEpoch` 仅为显式引用的会话状态，与持久选区分离；普通点击不 claim、不发布。页面提交更新引用目标的已保存 props/revision，删除目标清空附件；新面板仍被动启动。失败或等待确认不会因换选被误标为已同步。AI Skill 有附件时优先按附件 nodeId 读取最新页面，不用其他视觉选区替换它。
