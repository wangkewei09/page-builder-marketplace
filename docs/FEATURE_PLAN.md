...
已实现本地创建、打开、最近项目、独立文件保存、页面添加/切换/复制与保留历史页面的导入。默认画布文件已保留；无限画布平移/缩放/摆放以及 GitHub 克隆/拉取按钮仍待实现。可以打开已由现有 Git 工具克隆到本地的搭建器工程；普通源码仓库不能逆向为组件页面。详见 [项目方案](PROJECT_WORKSPACES.md)。

## 2026-09-24 / Figma 式项目卡片工作台

将项目管理弹窗替换为独立展示页：最近/所有/收藏/历史导航、搜索、封面卡片、按项目查看页面与单独设置。项目名称/240 字说明/手动封面/收藏写入项目清单，路径真实显示且可在文件夹移动后重新关联同一项目。保持本地目录与页面独立，未加入无限画布交互、在线 GitHub 克隆或自动截图。实现边界见 [项目方案](PROJECT_WORKSPACES.md)。

## 2026-09-24 / 系统文件夹选择

新建、打开和重新关联项目统一使用「选择文件夹」，选择后显示名称并折叠完整路径。无需手写路径，取消不改原值，显式确认后才创建/打开。当前系统适配为 macOS；不增加自动扫描。
