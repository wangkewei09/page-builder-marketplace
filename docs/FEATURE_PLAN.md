...
用户选中组件后，可直接在 Codex 中说“把这个按钮改成提交申请”“把这个区域改成两列”。界面显示当前 AI 操作对象，防止用户误以为改的是另一区域。
...
首版复用原组件 Renderer，避免产生第二套外观相似的组件。变体只保留一个权威字段，减少节点元信息与 props 互相矛盾的可能。组件内部 DOM、事件、键盘与焦点行为继续归组件所有。

## 9. 现有基础与缺口

以下为本次检查时的事实，不代表新功能已经完成。
...
8. 选中按钮后让 AI 修改“这个按钮”，AI 读取到正确节点；用户在 AI 工作期间修改同一页面时不会被旧数据覆盖。
