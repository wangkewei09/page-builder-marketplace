...
try {
  await page.goto(url, { waitUntil: "networkidle" }); assert.match(await page.title(), /页面搭建器/); await page.getByText("真实 B2B Renderer").waitFor();
  ...
  const label = page.locator('.inspector .field').filter({ hasText: "文案" }).locator("input"); await label.fill("提交申请"); await label.press("Tab"); await page.getByText("提交申请", { exact: true }).waitFor();
  const variant = page.locator('.inspector .field').filter({ hasText: "变体" }).locator("select"); await variant.selectOption("primary"); await page.locator('[data-component-reference="C-02"][data-button-variant="primary"]').first().waitFor();
  await label.focus(); await label.press("Delete"); assert.equal(await page.locator('.node-shell[data-renderer-valid="true"]').count(), 6);
  ...
  await page.getByLabel("添加分栏布局").click(); await page.getByText("分栏布局已添加").waitFor(); const columns = page.locator('.layout-shell.layout-columns').last(); await columns.click(); await page.getByLabel("添加标签").click(); await columns.locator('[data-component-reference="C-42"]').waitFor();
  ...
  await page.getByRole("button", { name: "预览" }).click(); const selectTrigger = page.locator('[data-component-reference="C-23"] [aria-expanded]'); await selectTrigger.click(); assert.equal(await selectTrigger.getAttribute("aria-expanded"), "true"); await page.getByRole("button", { name: "返回编辑" }).click();
  await page.getByRole("button", { name: "窄屏" }).click(); assert.match(await page.locator("#canvas-frame").getAttribute("class"), /is-mobile/);
