...
} catch (error) { await page.screenshot({ path: "/tmp/page-builder-browser-failure.png" }); console.error(JSON.stringify({ errors, body: await page.locator("body").innerText(), pages: await (await fetch(url + "/api/pages")).json() })); throw error; } finally { await browser.close(); child.kill("SIGTERM"); }
