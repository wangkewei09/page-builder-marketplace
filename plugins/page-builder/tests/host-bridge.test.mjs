...
try {
  ...
  const frame = page.frameLocator("#app"); await frame.getByRole("button", { name: "显示或隐藏属性面板" }).click(); await frame.getByText("点击组件后同步到对话").waitFor();
  ...
  await second.getByText("点击组件后同步到对话").waitFor();
  ...
  await frame.getByText("已同步到对话上下文").waitFor();
