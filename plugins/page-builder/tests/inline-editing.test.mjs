...
try {
  ...
  model = (await api(`/api/pages/${model.pageId}/operations`, { expectedRevision: model.revision, operations: [
    { type: 'add', parentId: model.root.id, node: { kind: 'component', componentId: 'C-02', props: { label: '保存订单' } } },
  ...
  const dialog = page.locator('.inline-editor');
  const dbl = async locator => { const box = await locator.boundingBox(); assert.ok(box); await page.mouse.dblclick(box.x + box.width / 2, box.y + box.height / 2); await page.locator('.inline-editor[data-ready="true"]').waitFor(); };
  const open = async (id, name) => { await shell(id).click({ position: { x: 2, y: 2 } }); await page.locator('#inspector:not([inert])').getByRole('button', { name: `编辑${name}`, exact: true }).click(); await page.locator('.inline-editor[data-ready="true"]').waitFor(); };
  await shell(button.id).click(); assert.equal(await dialog.count(), 0, 'single click only selects');
  ...
  assert.equal(await dialog.locator('[data-ui-renderer="C-21"]').count(), 1);
  await dialog.locator('input').fill('确认订单'); await dialog.locator('input').press('Enter');
  await dialog.waitFor({ state: 'detached' }); await settle(model.revision + 1);
  ...
  await open(button.id, '文案'); await dialog.locator('input').fill('取消的修改'); await dialog.locator('input').press('Escape');
  assert.equal((await saved()).revision, model.revision);
  await open(card.id, '卡片正文');
  await dialog.locator('textarea').fill('第一行\n第二行'); await dialog.locator('textarea').press('Enter');
  assert.equal((await saved()).revision, model.revision, 'Enter in multiline does not submit');
  await dialog.locator('textarea').press('Meta+Enter'); await dialog.waitFor({ state: 'detached' }); await settle(model.revision + 1);
  assert.match(model.root.children[1].props.body, /第一行\n第二行/);
  await open(card.id, '卡片标题');
  await dialog.locator('input').fill('中文标题');
  await dialog.locator('input').dispatchEvent('compositionstart');
  await dialog.locator('input').press('Enter'); assert.equal((await saved()).revision, model.revision, 'IME Enter does not save');
  await dialog.locator('input').dispatchEvent('compositionend');
  await page.mouse.click(5, 5); await dialog.waitFor({ state: 'detached' }); await settle(model.revision + 1);
  ...
  await dialog.locator('input').fill('8'); await dialog.locator('input').press('Enter'); await dialog.waitFor({ state: 'detached' }); await settle(model.revision + 1);
  assert.equal(model.root.children[2].props.value, 8);
  await dbl(shell(number.id).locator('input')); await dialog.locator('input').fill('not-a-number'); await dialog.locator('input').press('Enter');
  await dialog.getByText('请输入有效数字。').waitFor(); assert.equal((await saved()).revision, model.revision);
  await dialog.locator('input').press('Escape');
  // A failed source input mount still has a working Escape path outside the control.
  await shell(button.id).click();
  await page.evaluate(() => { window.normalRenderer = window.B2B.renderComponent; window.B2B.renderComponent = (spec, target) => { if (target.closest('.inline-editor')) return Promise.reject(new Error('inline mount rejection')); return window.normalRenderer(spec, target); }; });
  await page.locator('#inspector:not([inert])').getByRole('button', { name: '编辑文案', exact: true }).click();
  await dialog.getByText('输入控件加载失败，请关闭后重试。').waitFor();
  await page.keyboard.press('Escape'); await dialog.waitFor({ state: 'detached' });
  await page.evaluate(() => { window.B2B.renderComponent = window.normalRenderer; });
  assert.equal((await saved()).revision, model.revision);
  ...
  await open(button.id, '文案'); await dialog.locator('input').fill('重试内容'); await dialog.locator('input').press('Enter');
  await dialog.getByText('未保存：测试保存失败').waitFor(); assert.equal(await dialog.locator('input').inputValue(), '重试内容');
  assert.equal((await saved()).revision, model.revision); await page.unroute('**/operations');
  await dialog.locator('input').press('Enter'); await dialog.waitFor({ state: 'detached' }); await settle(model.revision + 1);
  ...
  await open(button.id, '文案'); await dialog.locator('input').fill('旧草稿');
  ...
  await dialog.locator('input').press('Enter'); await dialog.getByText('页面已被更新，草稿已保留。按 Esc 取消后重新编辑。').waitFor();
  assert.equal((await saved()).root.children[0].props.label, 'AI 修改'); assert.equal(await dialog.locator('input').inputValue(), '旧草稿');
  await dialog.locator('input').press('Escape'); await settle(model.revision);
  await open(tag.id, '文字'); await dialog.locator('input').fill('完成'); await dialog.locator('input').press('Enter'); await dialog.waitFor({ state: 'detached' }); await settle(model.revision + 1);
  // Preview keeps the source's original behavior and cannot open canvas editing.
  await page.getByRole('button', { name: '预览', exact: true }).click();
  const box = await shell(button.id).boundingBox(); await page.mouse.dblclick(box.x + 25, box.y + 15); assert.equal(await dialog.count(), 0);
  await page.getByRole('button', { name: '返回编辑', exact: true }).click();
  await open(card.id, '卡片标题'); await page.screenshot({ path: path.join(data, 'inline-desktop.png') });
  await dialog.locator('input').press('Escape');
  await page.setViewportSize({ width: 520, height: 800 });
  await shell(button.id).scrollIntoViewIfNeeded(); await dbl(shell(button.id).locator('[data-source-basic-button]'));
  const rect = await dialog.boundingBox(); assert.ok(rect.x >= 0 && rect.x + rect.width <= 520 && rect.y >= 0 && rect.y + rect.height <= 800);
  await page.screenshot({ path: path.join(data, 'inline-narrow.png') }); await dialog.locator('input').press('Escape');
  assert.deepEqual(errors.filter(e => !e.includes('400 (Bad Request)') && !e.includes('409 (Conflict)') && !e.includes('inline mount rejection')), []);
  const result = { passed: true, data, assertions: ['double click / single click', 'source C-21 editor', 'unique property paths', 'localized updates', 'Esc / multiline / IME / blur', 'numeric types', 'failure draft and retry', 'undo redo', 'revision conflict', 'preview', 'desktop and narrow'] };
