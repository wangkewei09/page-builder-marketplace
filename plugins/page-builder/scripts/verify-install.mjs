...
try {
 ...
 for (const name of ['project_choose_directory', 'project_directory_choice', 'project_cancel_directory_choice']) {
  assert.deepEqual(registered.find(tool => tool.name === name)?._meta?.ui?.visibility, ['app']);
 ...
 console.log(JSON.stringify({projectTools:true,projectSettings:true,appOnlyFolderTools:true,version,installed,matchingFiles:files.length,installedMcpResource:true,htmlCharacters:html.length,actualCodexHostReloaded:false}));
