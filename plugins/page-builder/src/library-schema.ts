...
import { parseInlineContract } from "./inline-protocol.js";
import { legacyInline } from "./inline-defaults.js";
...
export async function readLibraryCatalog(directory: string): Promise<Record<string, ComponentDefinition>> {
  ...
  try {
    const text = await readFile(path.join(directory, "components/runtime/inline-editing.json"), "utf8");
    if (Buffer.byteLength(text) > 1024 * 1024) throw new Error("画布编辑协议超过 1 MiB。");
    inline = parseInlineContract(JSON.parse(text), schemas);
  } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
