...
当前开发构建：**0.1.1+codex.20260922093539**。变更与已知边界见 [版本记录](CHANGELOG.md)。
...
双击画布中的按钮/标签文字、卡片标题/正文或单值输入框可直接编辑；Enter 保存单行，⌘/Ctrl+Enter 保存正文，失焦保存，Esc 取消。右侧「编辑…」可定位内容，其余变体和设置各出现一次。位置来自组件库独立 `inline-editing.json`，手动重载生效；没有可见位置的字段继续在右侧。源长文本控件上限为 240 字，输入时有提示。详见 [画布编辑说明](../../docs/INLINE_EDITING.md)。
